...
use Meow7\Phpimap\Client;
use Meow7\Phpimap\Exceptions\AuthFailedException;
use Meow7\Phpimap\Exceptions\ConnectionFailedException;
use Meow7\Phpimap\Exceptions\EventNotFoundException;
use Meow7\Phpimap\Exceptions\GetMessagesFailedException;
use Meow7\Phpimap\Exceptions\ImapBadRequestException;
use Meow7\Phpimap\Exceptions\ImapServerErrorException;
use Meow7\Phpimap\Exceptions\InvalidMessageDateException;
use Meow7\Phpimap\Exceptions\MessageContentFetchingException;
use Meow7\Phpimap\Exceptions\MessageFlagException;
use Meow7\Phpimap\Exceptions\MessageHeaderFetchingException;
use Meow7\Phpimap\Exceptions\MessageNotFoundException;
use Meow7\Phpimap\Exceptions\MessageSearchValidationException;
use Meow7\Phpimap\Exceptions\ResponseException;
use Meow7\Phpimap\Exceptions\RuntimeException;
use Meow7\Phpimap\IMAP;
use Meow7\Phpimap\Message;
use Meow7\Phpimap\Support\MessageCollection;

/**
 * Class Query
 *
 * @package Meow7\Phpimap\Query
 */
class Query {
    ...
    public function getUid(): array {
        $uidCollection = $this->search();
        return $uidCollection->toArray();
    }
    ...
    /**
     * Paginate the current query with a filter
     *
     * @param int $per_page
     * @param $page
     ...
     * @throws ResponseException
     */
    public function paginateFiltered($callback, int $per_page = 5, $page = null, string $page_name = 'imap_page'): LengthAwarePaginator
    {
        if ($page === null && isset($_GET[$page_name]) && $_GET[$page_name] > 0) {
            $this->page = intval($_GET[$page_name]);
        } elseif ($page > 0) {
        ...
        $this->limit = $per_page;
        $available_messages = $this->search();
        $available_messages_new = $available_messages->filter(function ($id) use ($callback) {
            return $callback($id);
        })->values();

        return $this->curate_messages($available_messages_new)->paginate($per_page, $this->page, $page_name, true);
    }
